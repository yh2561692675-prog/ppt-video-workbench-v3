...
def test_p05_rejects_manual_binding_outside_current_candidates() -> None:
    from pydantic import ValidationError
    from workbench.business_modules.p05_match.models import ContentMatchParameters

    page_id = uuid4()
    with pytest.raises(ValidationError, match="unknown page"):
        ContentMatchParameters.model_validate(
            {
                ...
                "manual_bindings": [
                    {"page_id": str(uuid4()), "selected_outline_ref": "paragraph:1"}
                ],
            }
        )
