...
registerEffect('ChapterCurtain', ({ plan }) => (
  <ChapterCurtain chapterTitle={plan.page_id} palette="blue" durationMs={plan.duration_ms} />
), { fallback: 'SafeSlide' });
registerEffect('ChartNarration', () => <ChartNarration series={[]} cuePoints={[]} annotation="" />, { fallback: 'SafeSlide' });
registerEffect('CompareMode', ({ plan }) => <CompareMode left={plan.page_id} right={plan.page_type} active="left" />, { fallback: 'SafeSlide' });
registerEffect('CardStack', ({ plan }) => <CardStack cards={[plan.page_id]} />, { fallback: 'SafeSlide' });
registerEffect('GaugeAndRatio', ({ plan }) => <GaugeAndRatio label={plan.page_id} value={0} />, { fallback: 'SafeSlide' });
registerEffect('PathBuilder', ({ plan }) => <PathBuilder nodes={[plan.page_id, plan.page_type]} />, { fallback: 'SafeSlide' });
registerEffect('TagMatrix', ({ plan }) => <TagMatrix tags={[plan.page_id, plan.page_type]} />, { fallback: 'SafeSlide' });
registerEffect('RiskAlert', ({ plan }) => <RiskAlert title={plan.page_id} reason={plan.page_type} />, { fallback: 'SafeSlide' });
registerEffect('MapHighlight', ({ plan }) => <MapHighlight points={[]} conclusion={plan.page_type} />, { fallback: 'SafeSlide' });
registerEffect('StatCounter', ({ currentFrame }) => <StatCounter spec={{ start: 0, end: 1, durationFrames: 30, format: 'integer' }} currentFrame={currentFrame} />, { fallback: 'SafeSlide' });
...
export function EffectInterpreter({ plan, currentFrame }: { plan: EffectPlanV2; currentFrame: number }) {
  const result = interpret(plan, 30);
  const Component = result.component;
  return <Component {...safeProps(plan, currentFrame)} />;
}

function fallback(issueCode: 'EFFECT_TEMPLATE_UNKNOWN' | 'EFFECT_TIMELINE_INVALID'): Interpretation {
