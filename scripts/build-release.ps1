...
try {
    ...
    uv run --frozen python $releaseArtifactsScript --repository-root $repoRoot --output $artifactManifestPath --installer $installerPath --payload-manifest (Join-Path $stageRoot "runtime-manifest.json") --candidate-id $CandidateId --version "0.1.1"
    if ($LASTEXITCODE -ne 0) {
