...
def validate_effect_plan(plan: Mapping[str, object] | EffectPlanV2) -> ValidationReport:
    ...
    if data.get("camera_count", 1) != 1:
        issues.append(ValidationIssue("two_main_cameras", "A page must have exactly one main camera."))

    caption = data.get("caption_rect")
    occupied = data.get("occupied_rects", [])
    if isinstance(caption, Mapping) and isinstance(occupied, Sequence):
        if any(_overlaps(caption, rect) for rect in occupied if isinstance(rect, Mapping)):
    ...
    cues = data.get("cues", [])
    if isinstance(speech_start, (int, float)) and isinstance(cues, Sequence):
        if any(isinstance(cue, Mapping) and cue.get("start_ms", 0) < speech_start for cue in cues):
    ...
    if isinstance(transition_duration, (int, float)) and transition_duration > 600:
        issues.append(ValidationIssue("transition_too_long", "Transition exceeds the 600 ms safety limit."))
    ...
    if isinstance(effects, Sequence) and any(
        ...
        issues.append(ValidationIssue("infinite_loop", "Infinite or unbounded loops are not allowed."))
